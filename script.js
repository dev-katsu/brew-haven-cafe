...
// Pause animation when hovering over the review section
...
// Floating Banner Pop-up Controller
document.addEventListener('DOMContentLoaded', () => {
  const popupAd = document.getElementById('popupAd');
  const closeAdBtn = document.getElementById('closeAdBtn');

  // Trigger popup when user scrolls past 300px
  window.addEventListener('scroll', () => {
    if (window.scrollY > 300 && !sessionStorage.getItem('adDismissed')) {
  ...
  if (closeAdBtn) {
    closeAdBtn.addEventListener('click', closePopupAd);
  }
});

// Dismiss popup and remember preference for the session
